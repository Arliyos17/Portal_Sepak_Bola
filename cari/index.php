...
<head>
	...
	<title>Pencarian Berita - SportNews</title>
		...
		<div class="row">
			<div class="col-md-12" style="background:white;height:200px auto;">
				<div class="row">
					...
					$kata = @$_POST['cari'];
					$total = mysqli_query($koneksi, "SELECT * FROM berita WHERE judul like '%$kata%' OR isi like '%$kata%'");
					...
					<div class="col-md-6" style="height:700px auto;">
						...
						<?php
						$kata = @$_POST['cari'];
						$str = "SELECT * FROM berita WHERE judul like '%$kata%' OR isi like '%$kata%' ORDER BY tanggal DESC LIMIT $awal, $hal";
						$query = mysqli_query($koneksi, $str) or die(mysqli_error());
						$jumlah = mysqli_num_rows($query);
						if ($jumlah == '0') {
							echo "Pencarian dengan kata kunci <b><i>$kata</b></i> tidak ditemukan";
						} else {
							while ($row = mysqli_fetch_array($query)) {
						?>
								<div class="media">
									<div class="media-left">
										<a href="../detail/?id=<?php echo $row['id_berita']; ?>">
											<img class="media-object" src="../images/berita/<?php echo $row['id_berita']; ?>.jpg" width="150px" height="100px alt=" ...>
										</a>
									</div>
									<div class="media-body">
										<font color="gray">Liga <?php echo $row['liga']; ?></font>
										<h4 class="media-heading"><a href="../detail/?id=<?php echo $row['id_berita']; ?>"><?php echo $row['judul']; ?></a></h4>
										<div class="media-body">
											<p>
											<div style="text-align:justify;"><?php echo substr($row['isi'], 0, 200); ?> ...</div><br><a href="../detail/?id=<?php echo $row['id_berita']; ?>">Baca Lengkap >></a></p>
										</div>
										<hr>
									</div>
								</div>
						...
						<nav>
							<ul class="pagination">
								<li>
									<?php if ($awal != 0) {
										echo "<a href='../cari/?awal=" . ($awal - $hal) . "' aria-label='Previous'>
							<span aria-hidden='true'>&laquo;</span>
								...
								<li>
									<?php for ($i = 0; $i <  $jum_hal; $i++) {
										$x = $i * $hal;
										if ($awal == $x) {
											echo "<span aria-hidden='true' class='disabled' style='color:black;'>" . ($i + 1) . "</span>";
										} else {
											echo " <a href='../cari/?awal=$x'>
							" . ($i + 1) . "
							</a>";
										}
									}
								...
								<li>
									<?php if ($awal != $i) {
										echo "<a href='../cari/?awal=" . ($awal + $hal) . "' aria-label='Next'>
